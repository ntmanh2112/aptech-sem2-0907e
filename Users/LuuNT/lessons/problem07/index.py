...
"""
	...
	#===================================
	# insert piclink
	def insert_piclink_form(self):
		a_names = ["piclink_url"]
		s = self.get_form("post", "/?answer=insert_piclink_form", "form_insert_link", "Create New Link Entry", a_names)
		self.whtml(s)
		
	#===================================	
	def do_insert_piclink_form(self):
	...
	#===================================
	def select_piclink(self):
		global g_login
		recs = self.sql_select("SELECT * FROM PicLink WHERE login_id = " + str(g_login.login_id) )
		sb = []
		for rec in recs:
			sb.append(str(rec.piclink_id)+"==="+str(rec.piclink_time)+"==="+str(rec.piclink_url)+"=== | <a href='/?answer=delete_piclink&piclink_id="+str(rec.piclink_id)+"'>Delete</a> | <a href='/?answer=update_link&piclink_id="+str(rec.piclink_id)+"'>Edit</a><br/><br/>")
		self.whtml("".join(sb))
	...
	def what_do_you_want(self):
		...
		if (self.can_respond()):
			...
			if (answer == "info"):
				...
				if ( self.prompt_login()): self.insert_piclink_form()
			elif (answer == "insert_piclink_form"):
				if ( self.prompt_login()): self.do_insert_piclink_form()
			elif (answer =="select_piclink"):
				if ( self.prompt_login()): self.select_piclink()
