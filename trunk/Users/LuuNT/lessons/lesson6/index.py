...
class Diary(db.Model):
	login_id = db.IntegerProperty()
	dairy_id = db.IntegerProperty()
	dairy_text = db.StringProperty()
	...
	#answer = register1:
		...
		#answer = yes:
			...
			#answer = no:
				...
				rec = self.sql_select1("SELECT * FROM Login0 WHERE login_username = '" + dna["login_username"]["value"] + "'")
				if (rec is not None):
				#answer = yes:
					#set valid = false
